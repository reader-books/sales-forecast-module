...
class Metrics(EnumType):
  ...
  SCORE = "score"
class Model(EnumType):
    LINEAR_REGRESSION = "Linear Reg"
    LINEAR = "linear"
    HOLT_WINTERS = "holt_winters"
    HOLT_WINTERS_REG = "Holt Winters Reg"
...
class ModelExample:
  def __init__(
    ...
    data: pd.DataFrame,
    model: str = None,
    period: int = Period.SEASON_MONTHLY,
    ...
    self.period = max(self.test_data, period)

  @property
  def forecast(self) -> dict[str, np.ndarray]:
    df = self.data.copy()
    results = {}
    forecaster = SalesForecaster(
      ...
      train_or_forecast=self.train_or_forecast,
    )
    try:
      ...
      logging.info("{0}:\tLinear regression failed: %s".format(str(__class__)), e)

    try:
      results[Model.STATE_SPACE] = (
        forecaster.state_space_forecast
      )
    except Exception as e:
      logging.info("{0}:\tState space forecast: %s".format(str(__class__)), e)

    try:
      results[Model.EXPONENTIAL_SMOOTHING] = (
        forecaster.exponential_smoothing_forecast
      )
    except Exception as e:
      logging.info("{0}:\tExponential smoothing forecast: %s".format(str(__class__)), e)

    try:
      results[Model.ARIMA] = (
        forecaster.arima_forecast)
    except Exception as e:
      logging.info("{0}:\tARIMA forecast failed: %s".format(str(__class__)), e)

    return results

  @property
  def evaluation(self) -> dict[str, float] | None:
    df = self.data.copy()
    results = {}
    if self.train_or_forecast:
      ...
      if forecast_data is None:
        return None
      for model, predicted in forecast_data.items():
        evaluator = ModelEvaluator(
          actual=data_test,
          predicted=predicted,  # !!!! Here !!! - period
          model_name=model)
  ...
  @property
  def visualization(self) -> dict[Any, Any]:
    ...
    if self.train_or_forecast:
      ...
      analysis_plot = (
        visualizer.plot_analysis_results)
      stats_table = (
        visualizer.plot_statistics_table)
      eval_table = (
        visualizer.plot_evaluation_table)

      if self.type_run == 1:
        self.figures[FiguresEnum.ANALYSIS] = analysis_plot
        self.figures[FiguresEnum.STATISTICS] = stats_table
      elif self.type_run == 2:
        self.figures[FiguresEnum.FORECAST] = forecast_plot
      else:
        self.figures[FiguresEnum.FORECAST] = forecast_plot
        self.figures[FiguresEnum.EVALUATION] = eval_table
    else:
      visualizer = SalesVisualizer(
        data=self.data,
        periods=self.period,
  ...
  def run_analysis(self) -> dict[str, Any]:
    df = self.data.copy()
    analyzer = SalesAnalyzer(data=df)

    test_error(self.data)

    if self.train_or_forecast:

      return {
        "data": self.data,
        ColumnData.GROWTH_RATE: analyzer.calculate_growth_rates,
        ColumnData.STATISTICS: analyzer.descriptive_statistics,
        "forecasts": self.forecast,
        "evaluation": self.evaluation,
        "figures": self.visualization
      }
    else:
      return {
        "data": self.data,
  ...
  def growth_rates(self) -> pd.DataFrame:
    df = self.data.copy()
    analyzer = SalesAnalyzer(data=df)
    results = analyzer.calculate_growth_rates
    return results

  @property
  def test_error_forecast_method(self) -> dict[str, np.ndarray]:
    df = self.data.copy()
    results = {}
    ...
    print(f"  Train data points: {self.train_data}")
    print(f"  Test data points: {self.test_data}")
    print(f"  Mode: {'TRAIN/TEST' if self.train_or_forecast else 'FORECAST'}")
    print(f"  Total data length: {len(df)}")

    # 2. Проверка SalesForecaster
    print(f"\nCreating SalesForecaster...")
    try:
      forecaster = SalesForecaster(
        data=df,
        periods=self.period,
      ...
      print(f"✓ SalesForecaster created successfully")

      # Проверяем атрибуты forecaster
      print(f"\nSalesForecaster attributes:")
      print(f"  forecaster.data type: {type(forecaster.data)}")
      if hasattr(forecaster.data, 'shape'):
        print(f"  forecaster.data shape: {forecaster.data.shape}")
      print(f"  forecaster.periods: {forecaster.periods}")
      print(f"  forecaster.train_data: {forecaster.train_data}")
      print(f"  forecaster.test_data: {forecaster.test_data}")
      print(f"  forecaster.train_or_forecast: {forecaster.train_or_forecast}")

    except Exception as e:
      print(f"✗ Failed to create SalesForecaster: {e}")
      import traceback
      traceback.print_exc()
      return results

    # 3. Тестируем каждую модель отдельно с детальной отладкой
    models_to_test = [
      (Model.LINEAR, "Linear Regression"),
      (Model.STATE_SPACE, "State Space"),
      (Model.EXPONENTIAL_SMOOTHING, "Exponential Smoothing"),
      (Model.ARIMA, "ARIMA"),
    ]
    ...
    for model_enum, model_name in models_to_test:
      ...
      try:
        # Получаем метод
        method_name = f"{model_enum.lower().replace(' ', '_')}_forecast"
        if hasattr(forecaster, method_name):
          print(f"  Method found: {method_name}")

          # Вызываем метод
          forecast_result = getattr(forecaster, method_name)
          ...
          if isinstance(forecast_result, np.ndarray):
            print(f"  ✓ {model_name} returned np.ndarray")
            print(f"    Shape: {forecast_result.shape}")
            print(f"    Dtype: {forecast_result.dtype}")
            print(f"    Min: {forecast_result.min():.2f}")
            print(f"    Max: {forecast_result.max():.2f}")
            print(f"    Mean: {forecast_result.mean():.2f}")
            print(f"    First 3 values: {forecast_result[:3]}")

            # Проверяем на NaN/Inf
            if np.any(np.isnan(forecast_result)):
              print(f"    ⚠ Contains NaN values: {np.isnan(forecast_result).sum()}")
            if np.any(np.isinf(forecast_result)):
              print(f"    ⚠ Contains Inf values")

            # Добавляем в результаты
            results[model_enum] = forecast_result
            print(f"  ✓ {model_name} added to results")
          else:
            print(f"  ✗ {model_name} returned wrong type: {type(forecast_result)}")
        else:
          print(f"  ✗ Method not found: {method_name}")

      except Exception as e:
        print(f"  ✗ {model_name} failed with error: {str(e)[:200]}")
        # Более детальная информация об ошибке
        import traceback
        error_details = traceback.format_exc()
        # Печатаем только первые 5 строк трассировки
    ...
    print(f"{'=' * 50}")
    if results:
      print(f"✓ Successfully generated forecasts for {len(results)} models:")
      for model, forecast in results.items():
        print(f"  {model}: shape={forecast.shape}, length={len(forecast)}")
    else:
      print(f"✗ NO FORECASTS GENERATED! All models failed.")
      print(f"\nPossible reasons:")
      print(f"  1. Check SalesForecaster implementation")
      print(f"  2. Check if models are properly defined")
      print(f"  3. Check data format in SalesForecaster methods")
      print(f"  4. Check for exceptions in model fitting")

    return results

def show_figure_blocking(fig) -> None:
  fig.canvas.mpl_connect(
    "button_press_event",
    lambda event: plt.close(fig))
  plt.show()

def test_error(data: pd.DataFrame = None) -> None:
  # В UI или в основном коде перед вызовом ModelExample
  print("=== DATA DIAGNOSTICS ===")
  print(f"Data type: {type(data)}")
  print(f"Data shape: {data.shape}")
  print(f"Columns: {list(data.columns)}")
  print(f"Date range: {data[ColumnData.DATE].min()} to {data[ColumnData.DATE].max()}")
  print(f"Number of days: {len(data)}")
  print(f"Sales - min: {data[ColumnData.SALES].min():.2f}, "
        f"max: {data[ColumnData.SALES].max():.2f}, "
        f"mean: {data[ColumnData.SALES].mean():.2f}")
  print(f"Any NaN in sales: {data[ColumnData.SALES].isnull().any()}")
  print(f"Zero variance: {data[ColumnData.SALES].std() == 0}")

  # Создаем ModelExample с отладкой
  model = ModelExample(
    data=data,
    period=90,  # Укажите явно
    train_or_forecast=True,
    type_run=2
  )

  # Проверяем forecast отдельно
  print("\n=== CHECKING FORECAST ===")
  forecasts = model.forecast
  print(f"Forecasts returned: {len(forecasts)} models")
...
class SalesAnalysisForecastUI:
  def __init__(self) -> None:
    ...
    ttk.Button(self.win,
               ...
               ).grid(row=30, rowspan=2, column=4)
